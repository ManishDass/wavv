    ...
    return (
        <div>
            <div className='flex justify-between items-center mt-5 mt-6 px-6 '>
                {options.left === 'search' ? (
                    <Search onClick={() => setSelectedTab('search')} className='z-20'/>
                ) : options.left === 'back' ? (
            ...
            {isModalOpen && (
                <div className='fixed h-screen w-screen bg-[#0F0817] z-50 top-0'>
                    <BgShape className='fixed w-screen bottom-10 z-10' />
                    <div className='flex justify-between items-center mt-[1.48rem] px-[1.21rem] '>
                        <div className='flex items-center justify-center bg-white bg-opacity-10 h-7 w-7 rounded-full back-button z-20' onClick={handleToggleModal} >
                            <BackIcon />
                        </div>
                        ...
                        <div className='flex items-center justify-center bg-white bg-opacity-10 h-7 w-7 rounded-full back-button' >
                            <div className='absolute h-14 w-14 -m-4 z-20' onClick={handleToggleModal} ></div>
                    ...
                    <ul className='text-white flex flex-col gap-4 font-sans text-sm mt-10'>
                        ...
                        <li className='flex justify-between p-3 w-screen px-6 z-20'>
                        ...
                        <p onClick={logout} className='font-santoshi-regular -mt-3 p-3 w-screen px-6 cursor-pointer z-20'>Logout</p>
                    </ul>
                </div>
            )}
        </div>
